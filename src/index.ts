...
export type SetConfigTransformCallback = (
  transform: TransformerFactory<SourceFile>
) => void;
export type ProjectTransformerFactory = (
  projectTransformerConfig: ProjectTransformerConfig
) => ProgramTransformerFactory;
export type ProgramTransformerFactory = (
  checker: TypeChecker,
  program: ts.Program
...
export function transformProject(
  ...
  host.createProgram = (names, opts, host, oldProgram, configDiag, refs) => {
    ...
    const newSources = transform(candidateFiles, [
      getTransformFactory(checker, program),
    ]);
    ...
    for (const file of newSources.transformed) {
      // TODO: This is broken if the root dir is above the tsconfig folder
  ...
  allConfigFiles.forEach((f) => {
    ...
    if (transformConfig.onTransformConfigFile) {
      const result = ts.transform(ts.parseJsonText(f, content), [
        transformConfig.onTransformConfigFile,
      ]);
      const printer = createPrinter(
        {},
        {
  ...
  const checkHost = createSolutionBuilderHost(
    sys,
    /*createProgram*/ undefined,
    (diag) => {
  ...
  const resultSolution = createSolutionBuilder(
    ...
    [
      path.join(
        path.resolve(outDir),
        path.relative(path.resolve(projDir), rootConfig)
      ),
    ],
    {}
  );
...
export function transformProjectFromNamespacesToModules(
  rootConfig: string,
  outDir: string
) {
  // We need to:
  ...
  transformProject(
    rootConfig,
    outDir + "_stage1",
    getExplicitifyTransformFactoryFactory
  );
  // 2. Strip all namespace declarations, ensure `export` modifiers are present, collect reexport files
  //   and add namespace imports
  transformProject(
    outDir + "_stage1/tsconfig.json",
    outDir + "_stage2",
    getStripNamespacesTransformFactoryFactory
  );
  // 3. Inline Imports
  transformProject(
    outDir + "_stage2/tsconfig.json",
    outDir,
    getInlineImportsTransformFactoryFactory
  );
}
